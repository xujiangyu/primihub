...
@ph.context.function(role='host', protocol='xgboost', datasets=['train_hetero_xgb_host'], port='8000', task_type="regression")
def xgb_host_logic(cry_pri="paillier"):
    ...
    if cry_pri == "paillier":
        ...
        xgb_host = XGB_HOST(n_estimators=num_tree, max_depth=max_depth, reg_lambda=1, min_child_sample=100,
                            sid=0, min_child_weight=1, objective='linear', channel=channel)
        channel.recv()
        y_hat = np.array([0.5] * Y.shape[0])
        for t in range(xgb_host.n_estimators):
